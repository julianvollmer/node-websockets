...
WebSocketBase.prototype._readFrame = function(data) {
    ...
    execChain(this.readChain.concat(done), frame.content);

    function done(next, result) {
        switch (frame.opcode) {
            case 0x00:
...
WebSocketBase.prototype._writeFrame = function(opcode, content) {
    ...
    execChain([start].concat(this.writeChain), new WebSocketFrame());
};

function execChain(chain, content) {
    var index = -1;
