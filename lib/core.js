...
WebSocketCore.prototype._read = function() {
    ...
    var incoming = this._incomings[this._incomings.length - 1];

    if (chunk === null)
        return this.push('');

    if (!incoming || !incoming._body || incoming._finish)
...
function createIncoming(stream) {
    var incoming = new WebSocketIncoming(stream);
    var i = stream._incomings.push(incoming) - 1;

    incoming._id = i;

    return stream._incomings[i];
}
