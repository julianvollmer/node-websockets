...
function WebSocketFrame(rawFrame) {
    // TODO: fix assignments (final).
    // it can be that final is false and will still be set to true
    this.final = parseFinal(rawFrame) || true;
    this.masked = parseMasked(rawFrame) || false;
...
// TODOS:
// - do not use signal words (final)
// - better semantic about unmask payload and setting unmasked payload (new api)
// - set masking key automatically if not set somewhere manuelly
// - update contstructor assignments
...
WebSocketFrame.prototype.setDecodedPayload = function(payload) {
    if (this.isMasked()) {
        this.setPayload(unmask(this.getMasking(), payload));
    } else {
...
WebSocketFrame.prototype.toBuffer = function() {
    // TODO: allow bigger lengths than 125
...
function parseLength(rawFrame) {
    ...
    if (len == 127) {
        // TODO: maybe we can add an extra function
        // which takes a buffer of variable length
        // and parses it as integer with the underneath
        // algorithm.
        len = rawFrame[9];
