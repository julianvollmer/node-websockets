...
WebSocketFrame.prototype.isValid = function() {
    if (this.rsv1 || this.rsv2 || this.rsv3)
        return new Error('Reserved flag should not be set in frame.');
    if (0x02 < this.opcode && 0x08 > this.opcode)
        return new Error('Opcode is in invalid range in range.');
...
WebSocketFrame.prototype.mask = function() {
    
};

WebSocketFrame.prototype.unmask = function() {
    if (!this.mask) return;
...
WebSocketFrame.prototype.mapFrame = function(frame) {
    ...
    this.frameLength = 2 + offset + maskingOffset + this.length;

    if (frame.length > this.frameLength) {
        this.glued = true;
        this.remnant = frame.slice(this.frameLength);
    }

    // if the frame is masked cut out the four bytes from 2 to 6 (with opt length offset)
    this.payload = frame.slice(2 + offset + maskingOffset, this.frameLength);
    this.masking = (this.mask) ? frame.slice(2 + offset, 6 + offset) : new Buffer(0);
    this.content = mask(this.masking, this.payload);
    this.frame = frame.slice(0, this.frameLength);
...
WebSocketFrame.prototype.toBuffer = function() {
    ...
    if (125 < this.length && 0xffff >= this.length) {
        ...
        if (0xffffffff < this.length) 
            throw new Error('Cannot handle UInt64BE');
