...
function getLength(frame) {
    ...
    switch (length) {
        case 126:
            ...
            length = frame.slice(2, 4).readUInt16BE(0);
            break;
        case 127:
            var lenBuf = frame.slice(2, 10);
            length = (lenBuf.readUInt32BE(0) << 8) + lenBuf.readUInt32BE(4);
...
function createHead(fin, mask, opcode, length) {
    ...
    header[0] = opcode | ((fin) ? 0x80 : 0x00);
    
    headers.push(header);
    
    if (length <= 125) {
        header[1] = length | ((mask) ? 0x80 : 0x00);
    }
    if (length > 125 && length <= 0xffff) {
        header[1] = 126 | ((mask) ? 0x80 : 0x00);
    ...
    if (length > 0xffff) {
        // TODO: throw error when reaching javascripts maximum
        header[1] = 127 | ((mask) ? 0x80 : 0x00);
        
        var lenBuf = new Buffer(8);
        
        lenBuf.fill(0);
        lenBuf.writeUInt32BE(length, 4);
