...
function readHeadBytes(state, head) {
    var headSize = state.headSize || calcHeadSize(head);
    
    state.fin = !!(head[0] & 0x80);
    state.mask = !!(head[1] & 0x80);
    state.rsv1 = !!(head[0] & 0x40);
    state.rsv2 = !!(head[0] & 0x20);
    state.rsv3 = !!(head[0] & 0x10);

    state.opcode = head[0] & 0x0f;
    state.length = head[1] & 0x7f;

    switch (state.length) {
        case 126:
            state.length = head.readUInt16BE(2);
            break;
        case 127:
            state.length = head.readUInt32BE(6);
            state.extendedLength = head.readUInt32BE(2);
            break;
    }

    if (state.mask)
        state.masking = head.slice(headSize - 4, headSize);
