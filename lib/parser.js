...
function writeHeadBytes(state, chunk) {
    ...
    if (length > 125 && length < 0x10000) {
        ...
        lengthBytes.writeUInt32BE(length, 6);
        length = 127;
    }
    ...
    if (!state.mask && state.masking && state.masking.length == 4) {
        var masking = state.masking.toString('base64');
        headBytes.write(masking, headSize - 4, headSize, 'base64');
    }

    if (state.mask && (!state.masking || state.masking != 4)) {
        var masking = crypto.randomBytes(4).toString('base64');
        headBytes.write(masking, headSize - 4, headSize, 'base64');
    }
