...
function bindToWebSocket(wsserver, wssocket) {
    wssocket.id = wsserver.sockets.push(wssocket) - 1;
    
    wssocket.on('pong', function(message) {
        wsserver.emit('pong', message, wssocket);
    });

    wssocket.on('close', function(message) {
        wsserver.sockets.splice(wssocket.id, 1);
        wsserver.emit('close', message, wssocket);
    });

    wssocket.on('message', function(message) {
        wsserver.emit('message', message, wssocket);
    });

    wssocket.on('stream:start', function() {
        wsserver.emit('stream:start', wssocket);
    });

    wssocket.on('stream:end', function() {
        wsserver.write(new Buffer(0));
        wsserver.emit('stream:end', wssocket);
    });
}
