...
WebSocketServer.prototype.listen = function(server) {
    var self = this;
    var extensionNames = this.extensionNames;

    server.on('upgrade', function(req, socket, head) {
        WebSocketUpgrade.serverUpgrade(req, socket, head, extensionNames, function(socket, options) {
            console.log('upgrading server: ', options);
            self.enabledExtensions = options.extensions;
