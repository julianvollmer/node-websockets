...
function bindToSource(wssocket, source, options) {
    ...
    wssocket._wscore.on('request', function(request) {
        ...
        wssocket._incoming.on('readable', function() {
            wssocket.read(0);
        });
...
function handleStream(wssocket, request) {
    console.log('request', request.fin, request.opcode, request.length);
    
    request.once('end', function() {
        request.removeAllListeners();

        console.log('request on end', request.fin, request.opcode, request.length);
