...
function parseChunk(chunk, context) {
    var wsframe, remnant;

    if (context.cache) {
        chunk = Buffer.concat([context.cache, chunk]);
        context.cache = null;
    }
    ...
    if (isGlued(chunk)) {
        remnant = remnant(chunk);
    }

    wsframe = new WebSocketFrame(chunk);

    // TODO: isValid should return Bool
    // TODO: validate should return Error or null
    if (!wsframe.isValid()) {
        parseFrame(wsframe, context);
    }

    if (remnant) {
        parseChunk(remnant, context);
    }
}

function parseFrame(wsframe, context) {
    // unmasking has to happen before
    // else most extensions overwriting
    // wsf.content will not work
    wsframe.unmask();

    execChain(context.readChain.concat(done), wsframe, context);
...
function isSplitted(chunk) {
    var wsf = new WebSocketFrame(chunk);

    return wsf.length > wsf.payload.length;
}

function isGlued(chunk) {
    var wsf = new WebSocketFrame(chunk);

    return wsf.glued;
}

function remnant(chunk) {
    var wsf = new WebSocketFrame(chunk);
