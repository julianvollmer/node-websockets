...
WebSocketStream.prototype._write = function(chunk, encoding, done) {
    var state = this.writeFrameState;
...
WebSocketStream.prototype.writeHead = function(head) {
    var state = this.writeFrameState;
    ...
    if (head.opcode < 0x0b && 
       (head.opcode < 0x02 || head.opcode > 0x07))
...
function parseBody(stream, state, chunk) {
    ...
    stream.push(mask(state.masking, payload, state.index));
